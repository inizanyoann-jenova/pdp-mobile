# Export PDF Optimal — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Réécrire `src/utils/exportPdf.js` pour produire une fiche de visite préalable en 2–3 pages max (infos chantier + tableau synthétique risques/mesures + signatures), en remplaçant le PDF actuel de 6–8 pages.

**Architecture:** Un seul fichier modifié (`src/utils/exportPdf.js`), découpé en fonctions pures : `buildAttentionPoints` (testable, jointure questions/mesures via `questionId`), `renderPage1/2/3` (rendu jsPDF), `addPageHeader`/`addPageFooter` (communs à toutes les pages). L'interface publique `exportPdP(pdp, opts, settings)` reste identique — aucune autre fichier modifié.

**Tech Stack:** jsPDF 2.5, Vitest (déjà configuré), React 18 / Vite 5
...
| Statut | Fichier | Rôle |
|--------|---------|------|
| Modifier | `src/utils/exportPdf.js` | Réécriture complète — seule modification |
| Créer | `src/tests/exportPdf.test.js` | Tests unitaires de `buildAttentionPoints` |

---

## Task 1 — Tests pour `buildAttentionPoints`

**Files:**
- Create: `src/tests/exportPdf.test.js`

`buildAttentionPoints(pdp)` est la seule fonction pure du module : elle calcule les lignes du tableau de risques en joignant `reponses` (NON/NSP) avec les mesures sélectionnées via `questionId`. C'est la seule partie testable sans mocker jsPDF.
...
- [ ] **Step 2: Lancer les tests — vérifier qu'ils échouent (fonction pas encore créée)**

```bash
npm test -- --reporter=verbose
```

Attendu : erreur `buildAttentionPoints is not a function` ou `Cannot find module`.

---

## Task 2 — Implémenter `buildAttentionPoints` dans `exportPdf.js`

**Files:**
- Modify: `src/utils/exportPdf.js`

Cette tâche ajoute uniquement `buildAttentionPoints` en haut du fichier existant (export nommé). Le reste du fichier reste intact pour l'instant.

- [ ] **Step 1: Ajouter `buildAttentionPoints` comme export nommé**

Lire le fichier `src/utils/exportPdf.js`, puis ajouter juste après la ligne `import { ... } from './risques';` (ligne 3) :
...
Attendu : 9 tests PASS pour `buildAttentionPoints`.

- [ ] **Step 3: Commit**

```bash
git add src/utils/exportPdf.js src/tests/exportPdf.test.js
git commit -m "feat(pdf): add buildAttentionPoints + tests"
```

---

## Task 3 — Réécrire `exportPdf.js` complet

**Files:**
- Modify: `src/utils/exportPdf.js` (remplacement total du contenu)

Cette tâche remplace entièrement le fichier. Le résultat produit un PDF 3 pages : page 1 (chantier + score), page 2 (tableau risques), page 3 (signatures).

- [ ] **Step 1: Remplacer `src/utils/exportPdf.js` par le contenu suivant**
...
async function renderPage2(doc, pdp, points, ctx) {
  ...
  if (points.length === 0) {
    ...
    // Colonnes : cat=30, risk=68, level=18, measure=66 (total=182 = inner)
...
export async function exportPdP(pdp, { returnBlob = false } = {}, settings = {}) {
  ...
  const ctx    = { W, H, margin, inner, primaryRgb, logoBase64, companyName, refPdp };
  ...
  // Page 2 (peut s'étendre sur plusieurs pages si nombreux risques)
  doc.addPage();
  doc.setFillColor(...C.bg); doc.rect(0, 0, W, H, 'F');
  addPageHeader(doc, ctx);
  await renderPage2(doc, pdp, points, ctx);

  // Page 3 — toujours une nouvelle page après page 2
  doc.addPage();
  doc.setFillColor(...C.bg); doc.rect(0, 0, W, H, 'F');
  addPageHeader(doc, ctx);
...
Attendu : 9 tests PASS. Si un test échoue, vérifier que `buildAttentionPoints` est bien exportée dans le nouveau fichier.

---

## Task 4 — Vérification manuelle & commit final
...
- [ ] **Step 2: Ouvrir un PDP existant dans le navigateur**

Naviguer vers un plan avec des réponses variées (au moins 2 NON, 1 NSP, des mesures sélectionnées, des photos si possible).

- [ ] **Step 3: Cliquer sur "Exporter PDF" et vérifier le document**

Checklist de vérification :
- [ ] PDF a exactement 3 pages (ou 4 si beaucoup de photos)
- [ ] Page 1 : logo/initiales en en-tête, bloc chantier 2 colonnes, badge niveau de risque coloré, mini-barres catégories
- [ ] Page 2 : titre "RISQUES IDENTIFIÉS & MESURES DE PRÉVENTION", tableau avec en-tête bleu foncé, 1 ligne par NON/NSP, badge rouge/amber, mesures en dernière colonne
- [ ] Page 3 : texte d'engagement en italique, 2 blocs signature côte à côte, bandeau récap bleu en bas
- [ ] Pied de page sur chaque page : "Document confidentiel · Page N / Total"
- [ ] Référence PDP en haut à droite de l'en-tête si le plan a un ID
- [ ] Si 0 risques non-conformes : bloc vert "Aucun point critique" page 2

- [ ] **Step 4: Tester cas limite — plan vide (brouillon sans réponses)**

Créer ou ouvrir un plan sans réponses, exporter → le PDF doit s'ouvrir sans erreur JS, page 2 doit afficher le bloc vert.

- [ ] **Step 5: Commit final**

```bash
git add src/utils/exportPdf.js
git commit -m "feat(pdf): rewrite exportPdf — fiche visite préalable 3 pages"
```
